  ...
  return (
    <div className="relative h-[600px] md:h-[700px] overflow-hidden font-roboto" id="home">
            ...
            <div className="h-full bg-gradient-to-b from-black/40 via-black/50 to-black/60 flex items-center justify-center">
              <div className="text-center text-white max-w-4xl px-4">
                ...
                <Button 
                  ...
                  className="group relative overflow-hidden bg-gradient-to-r from-orange-600 via-red-600 to-orange-700 hover:from-orange-700 hover:via-red-700 hover:to-orange-800 text-white px-10 py-4 text-lg font-semibold shadow-2xl transition-all duration-300 hover:-translate-y-1 animate-fade-in animation-delay-400 border-0 rounded-2xl"
                >
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`relative transition-all duration-300 ${
              index === currentSlide 
                ? 'w-8 h-3 bg-gradient-to-r from-orange-500 to-red-500 rounded-full shadow-lg' 
                : 'w-3 h-3 bg-white/50 hover:bg-white/70 rounded-full hover:scale-110'
            }`}
          >
            {index === currentSlide && (
              <div className="absolute inset-0 bg-gradient-to-r from-orange-400 to-red-400 rounded-full blur-sm opacity-60"></div>
            )}
          </button>
        ))}
      </div>
      ...
      <button
        ...
        className="absolute left-6 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center text-white hover:bg-gradient-to-r hover:from-orange-500/80 hover:to-red-500/80 transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-xl group"
      >
        <ChevronLeft className="w-6 h-6 group-hover:-translate-x-0.5 transition-transform duration-300" />
      </button>
      <button
        onClick={() => setCurrentSlide((prev) => (prev + 1) % slides.length)}
        className="absolute right-6 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center text-white hover:bg-gradient-to-r hover:from-orange-500/80 hover:to-red-500/80 transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-xl group"
      >
